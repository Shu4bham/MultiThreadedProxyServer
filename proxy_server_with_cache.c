...
void *thread_fn(void *socketNew)
{
	...
	strcpy(tempReq, buffer); // Properly copies the string with null terminator
	...
	if (temp == NULL)
	{
		printf("Some error finding from cache");
	}
	if (temp != NULL)
	{
...
int main(int argc, char *argv[])
{
	...
	proxy_socketId = socket(AF_INET, SOCK_STREAM, 0); // AF_INET = IPV4 and SOCK_STREAM = TCP
...
int add_cache_element(char *data, int size, char *url)
{
	...
	else
	{
		...
		element->url = (char *)malloc(strlen(url) + 1); // +1 for '\0'
		strcpy(element->url, url);						// Ensures null termination
